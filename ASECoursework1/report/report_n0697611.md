...
The second stage of the algorithm continues inductively, making a number of passes, and on each pass sorting the lists. 

Because the number of items to search is reduced by $t$ each time, and $t$ doubles each time, the number of items to search reduces by twice as much as previously each time. As soon as $t≥N$, there will be no items left to search, and no more passes will be made. Because $t$ doubles on each pass, the number of passes will be O(log₂ N). As the time complexity of searching the list is only O(log₂ N) on any given pass ($t$ being fixed for that pass), and the number of passes scales logarithically with N, the overall time complexity for the second stage will be O((log₂ N)²).

The number of items to search for a given pass with a given $t$, with $N$ being the total number of names:

<img src="assets/f1.png" width="120"/>
...
In the original problem, it is claimed that it is possible to reconstruct the original order in fewer than 1000 passes for 3,000,000 names. 
