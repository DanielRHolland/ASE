...
The methodology used could be improved by alternating the running of each function, instead of running all twenty-five runs of all sizes for one, followed by the same for the other.
...
Combination 1 grew at a very close to linear rate, and it was predicted to be of time-complexity O(n). This curve is more prominent for smaller values of n (see Appendix A). The slight curve could be the result of imperfections in measurements
...
The spread of results is greater for 
...
In the first stage of the algorithm, the file is copied and the copies are sorted. Initially copying the file is O(n), then sorting the file and its copy is O(n log₂ n). The overall compexity is therefore the greatest of these, which is O(n log₂ n). 

The second stage of the algorithm continues inductively, making a number of passes, and on each pass sorting the lists. Because the number of items to search is reduced by $t$ each time, and $t$ doubles each time, the number of items to search reduces by twice as much as previously each time. As soon as $t≥n$, there will be no items left to search, and no more passes will be made. Because $t$ doubles on each iteration, the number of iterations will be O(log₂ n). As the time complexity of searching the list is only O(log₂ n) on any given pass ($t$ being fixed for that pass), and the number of passes scales logarithically with n, the overall time complexity for the second stage will be O((log₂ n)²). The provided solution states this to be the order of the number of passes.

As the first stage of the algorithm has a time complexity of O(n log₂ n), and the second stage has a time complexity of O((log₂ n)²), the overall time complexity is O(n log₂ n), as this is the greater of the two.

In the original problem, it is claimed that it is possible to reconstruct the original order in fewer than 1000 passes for 3,000,000 names. The second stage requires $log₂n$ (rounded up to next integer) iterations, $log₂(3,000,000)=21.52$. Therefore there will be 21 full iterations, and 1, final, partial, iteration. 
...
The results show a dramatic difference between the Serial and Non-Serial solutions. The Serial Solution is clearly slower, 
...
![Means over twenty-five runs, except for Serial Algorithm values for 3,000,000 and 2,000,000, which are the values for the sole reading](assets/vs3M_t.svg)
...
Standard for Programming Language C ++, C++ International Standard [https://isocpp.org/](https://isocpp.org/)
