...
The items must be inserted into and then searched for in the first data structure. `map` has a guaranteed time complexity of O(log₂ n) for both inserts and searches. The best case time complexity for searching a map is O(1), but as every item in the map must eventually be searched for in this algorithm anyway, this does not affect the best case time complexity of the algorithm. The second data structure only requires that items are inserted at the front and back, before the whole sequence is iterated through once at the end to output the final order. `list` has a time complexity of O(1) to insert at the front and back, making it well suited to this task.  
The algorithm requires the insertion of all elements into the map (O(n log₂ n)), the retrieval by search of each element from the map (O(n log₂ n)), then the insertion of all elements into the list at either the front or back (O(n)). The greatest of these is O(n log₂ n), therefore the complexity of the algorithm is O(n log₂ n) overall. 
